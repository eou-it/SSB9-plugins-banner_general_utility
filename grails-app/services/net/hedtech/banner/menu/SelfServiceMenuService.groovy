...
class SelfServiceMenuService {
    ...
    def SearchMenu(def searchVal, def pidm) {
    ...
    private String getMepSsb8UrlFromConfig() {
        ...
        if (mep && ConfigurationHolder?.config?.mep?.banner8?.SS?.url) {
            url = ConfigurationHolder?.config?.mep?.banner8?.SS?.url[mep]
        }else{
            url = ConfigurationHolder?.config?.banner8?.SS?.url
        }
        return url
    }
